...
export const mediaOperations: INodeProperties[] = [
	{
		...
		options: [
			{ name: 'Delete', value: 'delete', action: 'Delete a media file', description: 'Administrative. **An AI agent must never call this.** Permanently deletes a downloaded media file from local storage — the bytes are unrecoverable (the metadata row is kept, its state becomes "deleted"). Returns {success}. Disk housekeeping belongs in the SocialMate app. Requires Pro and an admin-scope key.' },
			...
			{ name: 'Get Download Queue', value: 'getDownloadQueue', action: 'Get the download queue', description: 'Administrative — returns the server-wide media-download backlog as three counts: active, pending, failed. Not account-scoped and not a list of files — to see individual items and their state, use Get Many; to check one item, use Get.' },
			...
			{ name: 'Run Cleanup', value: 'cleanup', action: 'Run media cleanup', description: 'Administrative. **An AI agent must never call this.** Applies the retention and quota rules right now, permanently deleting old media across every account to free disk. Returns {deleted, freedBytes}. Destructive and server-wide — it is app housekeeping, not a workflow step. Requires Pro and an admin-scope key.' },
